 ...
 *
 */
public class RightFragment extends Fragment implements OnClickListener, OnCheckedChangeListener, OnInteractListener {
	...
	private void initFaceEffectView() {
		String items[] = getActivity().getResources().getStringArray(R.array.jazzy_effects_ch);
		mFaceEffectView = mInflater.inflate(R.layout.face_jazz_effect_view, null);
		mFaceEffect = (WheelView) mFaceEffectView.findViewById(R.id.face_effect);
		mFaceEffectBtn = (Button) mFaceEffectView.findViewById(R.id.face_effect_btn);
		mFaceEffectBtn.setOnClickListener(this);
		ArrayWheelAdapter<String> adapter = new ArrayWheelAdapter<String>(getActivity(), items);
	...
	private void initExitView() {
		...
		mComfirmExitBtn = (Button) mExitConfirmView.findViewById(R.id.confirm_exit_btn);
	...
	private void initAboutView() {
		...
		TextView app_information = (TextView) mAboutView.findViewById(R.id.app_information);
		String myBlog = String.format("<a href=\"%s\">%s</a>", getString(R.string.my_blog_url), getString(R.string.my_blog_url));
		app_information.setText(Html.fromHtml(getString(R.string.app_information, myBlog)));
	...
	private void initView(View view) {
		...
		mMsgNotifySwitch = (SwitchButton) view.findViewById(R.id.message_notify_switch);
		mMsgSoundSwitch = (SwitchButton) view.findViewById(R.id.message_sound_switch);
		mPullRefreshSoundSwitch = (SwitchButton) view.findViewById(R.id.pullrefresh_sound_switch);
		mShowHeadSwitch = (SwitchButton) view.findViewById(R.id.show_head_switch);
	...
	public void onClick(View v) {
		...
		case R.id.acount_set_btn:
			...
			User u = new User(mSpUtil.getUserId(), mSpUtil.getChannelId(), nick, mHeadWheel.getCurrentItem(), 0);
			mUserDB.addUser(u);
			Message msgItem = new Message(System.currentTimeMillis(), "hi", mSpUtil.getTag());
			new SendMsgAsyncTask(mApplication.getGson().toJson(msgItem), "").send();// 发送信息更新一下
			if (mSlidingLayer.isOpened()) {
				mSlidingLayer.closeLayer(true);
			}
			break;
