 ...
 *
 */
public class MainActivity extends BaseSlidingFragmentActivity implements OnClickListener, PushMessageReceiver.EventHandler, OnHomePressedListener, OnPullRefreshSwitchListener {
		...
		public void handleMessage(Message msg) {
			...
			case NEW_FRIEND:
				...
				if (mLeftFragment == null)
					mLeftFragment = (LeftFragment) getSupportFragmentManager().findFragmentById(R.id.main_left_fragment);
			...
			case NEW_MESSAGE:
				...
				if (mUserDB.selectInfo(userId) == null) {// 如果不存在此好友，则添加到数据库
					User user = new User(userId, msgItem.getChannel_id(), nick, headId, 0);
					mUserDB.addUser(user);
					mLeftFragment = (LeftFragment) getSupportFragmentManager().findFragmentById(R.id.main_left_fragment);
					mLeftFragment.updateAdapter();// 更新一下好友列表
				}
				// TODO Auto-generated method stub
				MessageItem item = new MessageItem(MessageItem.MESSAGE_TYPE_TEXT, nick, System.currentTimeMillis(), content, headId, true, 1);
				mMsgDB.saveMsg(userId, item);
				// 保存到最近会话列表
				RecentItem recentItem = new RecentItem(userId, headId, nick, content, 0, System.currentTimeMillis());
	...
	private void initView(Bundle savedInstanceState) {
		...
		mRecentListView.setSwipeListViewListener(new BaseSwipeListViewListener() {
			@Override
