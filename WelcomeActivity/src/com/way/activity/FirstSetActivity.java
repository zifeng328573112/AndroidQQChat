 ...
 *
 */
public class FirstSetActivity extends Activity implements OnClickListener, PushMessageReceiver.EventHandler {
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case LOGIN_OUT_TIME:
				...
				if (mConnectServerDialog != null && mConnectServerDialog.isShowing())
	...
	public void onClick(View v) {
		...
		case R.id.first_start_btn:
			...
			PushManager.startWork(getApplicationContext(), PushConstants.LOGIN_TYPE_API_KEY, PushApplication.API_KEY);// 无baidu帐号登录,以apiKey随机获取一个id
	...
	public void onBind(String method, int errorCode, String content) {
		...
		if (errorCode == 0) {// 如果绑定账号成功，由于第一次运行，给同一tag的人推送一条新人消息
			User u = new User(mSpUtil.getUserId(), mSpUtil.getChannelId(), mSpUtil.getNick(), mSpUtil.getHeadIcon(), 0);
			mUserDB.addUser(u);// 把自己添加到数据库
			com.way.bean.Message msgItem = new com.way.bean.Message(System.currentTimeMillis(), "hi", mSpUtil.getTag());
