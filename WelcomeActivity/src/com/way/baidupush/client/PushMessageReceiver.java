...
public class PushMessageReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (intent.getAction().equals(PushConstants.ACTION_MESSAGE)) {
			...
			L.i("onMessage: method : " + method + ", result : " + errorCode + ", content : " + content);
	...
	private void showNotify(Message message) {
		...
		notification.setLatestEventInfo(PushApplication.getInstance(), application.getSpUtil().getNick() + " (" + mNewNum + "条新消息)", tickerText, contentIntent);
		// 下面是4.0通知栏api
			...
			if (NetUtil.isNetConnected(context)) {
				if (errorCode == 30607) {
					...
					new Handler().postDelayed(new Runnable() {
						...
						public void run() {
							// TODO Auto-generated method stub
							PushManager.startWork(context, PushConstants.LOGIN_TYPE_API_KEY, PushApplication.API_KEY);
						}
					}, 2000);// 两秒后重新开始验证
				}
			} else {
