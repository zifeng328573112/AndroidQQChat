...
public class RecentDB {
	...
	public RecentDB(Context context) {
		...
		db.execSQL("CREATE table IF NOT EXISTS " + RECENT_TABLE_NAME + " (_id INTEGER PRIMARY KEY AUTOINCREMENT,userId TEXT, name TEXT, img TEXT,time TEXT,num TEXT,message TEXT)");
	}
	...
	public void saveRecent(RecentItem item) {
		if (isExist(item.getUserId())) {
			...
			db.update(RECENT_TABLE_NAME, cv, "userId=?", new String[] { item.getUserId() });
		} else {
			db.execSQL("insert into " + RECENT_TABLE_NAME + " (userId,name,img,time,num,message) values(?,?,?,?,?,?)",
					new Object[] { item.getUserId(), item.getName(), item.getHeadImg(), item.getTime(), item.getNewNum(), item.getMessage() });
		}
	}
	...
	public LinkedList<RecentItem> getRecentList() {
		...
		while (c.moveToNext()) {
			...
			RecentItem item = new RecentItem(userId, icon, name, message, num, time);
			list.add(item);
		}
	...
	private boolean isExist(String userId) {
		Cursor c = db.rawQuery("SELECT * FROM " + RECENT_TABLE_NAME + " WHERE userId = ?", new String[] { userId });
